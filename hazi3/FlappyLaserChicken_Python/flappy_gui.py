...
class FlappyGUI:
    def __init__(self, agent: FlappyAgent, n_iterations, env: Environment, env_scaler=10, train_update_interval=50,
    ...
    def draw(self):
        ...
        for tube in tubes['list']:
            ...
            self.canvas.create_rectangle(
                ...
                fill='yellow')
        for eagle in eagles['list']:
            self.canvas.create_rectangle(
                eagle[0] * self.env_scaler,
                eagle[1] * self.env_scaler,
                (eagle[0] + eagles['size']) * self.env_scaler,
                (eagle[1] + eagles['size']) * self.env_scaler,
                fill='brown'
            )
        if shot_fired:
            self.canvas.create_rectangle(
                bird_x + bird_size,
                bird_y,
                self.env.map_size.x * self.env_scaler,
                bird_y + bird_size,
                fill='red'
            )
